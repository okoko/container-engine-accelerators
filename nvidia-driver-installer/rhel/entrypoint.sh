...
download_kernel_src() {
  ...
  # For download we need the subscription. These symlinks are according
  # to /usr/share/containers/mounts.conf on RHEL to satisfy symlinks in
  # the ubi.
  mkdir -p /run/secrets
  ln -s /root/etc/pki/entitlement /run/secrets/etc-pki-entitlement
  ln -s /root/etc/yum.repos.d/redhat.repo /run/secrets/redhat.repo
  ln -s /root/etc/rhsm /run/secrets/rhsm
